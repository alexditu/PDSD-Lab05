...
public class AddressBookActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		FragmentTransaction fragmentTransaction = fragmentManager
				.beginTransaction();
		fragmentTransaction.add(R.id.contact_basic_details,
				new ContactBasicDetailsFragment());
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		if (id == R.id.action_update) {
			...
			if ((contactPhoneNumbersParts.length % 2) == 0) {
				...
				while (index < contactPhoneNumbersParts.length) {
					PhoneNumber contactPhoneNumber = new PhoneNumber(
							contactPhoneNumbersParts[index + 1],
							contactPhoneNumbersParts[index]);
			...
			String contactEmailAddresses = contactEmailsTextView.getText()
					.toString();
			String[] contactEmailAddressesParts = contactEmailAddresses
					.replaceAll(" ", "").split("[:\n]");
			if ((contactEmailAddressesParts.length % 2) == 0) {
				...
				while (index < contactEmailAddressesParts.length) {
					EmailAddress contactEmailAddress = new EmailAddress(
							contactEmailAddressesParts[index + 1],
							contactEmailAddressesParts[index]);
					contactEmails.add(contactEmailAddress);
					index += 2;
				}
				intentUpdate.putExtra(Constants.CONTACT_EMAILS, contactEmails);
			}
			intentUpdate.putExtra(Constants.CONTACT_ID,
		...
		if (id == R.id.action_delete) {
			...
			contentProviderOperations.add(ContentProviderOperation
					...
					.withSelection(RawContacts.CONTACT_ID + " = ?",
							new String[] { String
									.valueOf(addressBookContactAdapter
											.getContactId()) }).build());
			try {
				...
				Log.e(Constants.TAG, "An exception has occurred: "
						+ operationApplicationException.getMessage());
				if (Constants.DEBUG) {
			...
			FragmentManager fragmentManager = getFragmentManager();
			ContactAdditionalDetailsFragment contactAdditionalDetailsFragment = (ContactAdditionalDetailsFragment) fragmentManager
					.findFragmentById(R.id.contact_additional_details);
	...
	public void onActivityResult(int requestCode, int resultCode, Intent intent) {
		...
		case Constants.OPERATION_INSERT:
			if (resultCode == Activity.RESULT_OK) {
				Toast.makeText(this, "Insert operation succeeded",
						Toast.LENGTH_LONG).show();
			} else {
				Toast.makeText(this, "Insert operation failed",
						Toast.LENGTH_LONG).show();
			}
			break;
		case Constants.OPERATION_UPDATE:
			...
			if (resultCode == Activity.RESULT_OK) {
				Toast.makeText(this, "Update operation succeeded",
						Toast.LENGTH_LONG).show();
			} else {
				Toast.makeText(this, "Update operation failed",
						Toast.LENGTH_LONG).show();
			}
			break;
		}
